      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6">
            ...
            <p className="m-2 text-(3.5 text2)">
              点击或拖拽{multiple ? "一个或多个文件" : "文件"}上传
            </p>
        ...
        <div className="flex">
          <h2 className="text-lg font-bold">
            文件列表{selectedFiles?.length && `(${selectedFiles?.length})`}
          </h2>
        ...
        <div className="flex flex-col flex-1 overflow-auto mt-2 hide-scrollbar">
          ...
          <div className="flex flex-col gap-2">
            {selectedFiles.map((file) => (
              <div
                key={file.Name}
                className="flex items-center border-2 border-solid border-border rounded-lg p-2"
              >
                <div className="flex items-center gap-2 truncate flex-1">
                  <div
                    className={cn(
                      "text-text",
                      FileTypeList.find((item) => item.type === file.Ext)
                        ?.icon || "i-tabler:file"
                    )}
                  ></div>
                  <div className="flex flex-col line-height-1em">
                    <span className="truncate font-bold text-(3.5 text)">
                      {file.Name}
                    </span>
                    <span className="text-(3 text2)">
