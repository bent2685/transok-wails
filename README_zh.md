...
- [x] 深色模式
- [x] 纯文字分享
- [x] 文件夹分享（加入整个文件夹；被分享方可逐层浏览子目录、逐个下载文件或将当前目录打包为 zip）
- [x] 文件夹内容动态更新（每次浏览实时读取，自动反映磁盘上的增删改）
- [x] 在系统文件管理器中打开共享文件夹（Finder / 资源管理器 / 文件）
- [x] Finder 风格下载网格（图片缩略图、备注角标、可折叠搜索、分类筛选）
